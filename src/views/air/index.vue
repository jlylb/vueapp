<template>
 <div class='layout-container'>
  ...
  <mt-cell :title="item.label" is-link  v-for='(item, index) in selectCity' :key='index' @click.native='openDetail(item, index)'>
...
export default {
  ...
  methods: {
    ...
    openDetail(item, index) {
      this.$router.push({name: 'air_device2', params: { dapeng: item.label, areaId: item.value } })
    },
    setProvince() {
