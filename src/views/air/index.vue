...
export default {
  ...
  methods: {
    selectButton(tab) {
      ...
      if(tab==='tab-temp') {
        this.$nextTick(_ => {
          this.$refs.tabTemp.echarts.resize()
        })
      }
    },
    formatChartData() {
      const { rd_air_backwindtemp, rd_air_backwindhum, rd_air_sendwindtemp, rd_air_sendwindhum, rd_air_innertemp, rd_air_innerhum } = this.detail
      const rows = [
      ...
      let yAxisName = ['温度: °C', '湿度: %'], yAxisType = ['value', 'value'], axisSite = { right: ['wet']}
      let min = [0, 0], max = [100, 100], labelMap = { temp: '温度', wet: '湿度' }
