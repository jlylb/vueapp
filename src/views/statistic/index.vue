<template>
  <div class="layout-container">
    <top-component @top-btn="filterChart"></top-component>
    ...
    <mt-popup v-model="filterLeft" position="right" class="popup-menu-right">
      ...
      <mt-radio title="查询时间" class="radio-time" v-model="selectDate" :options="radioOptions"></mt-radio>
    ...
    >
      <mt-cell
        ...
        :class="['dapeng-item', { itemSelect: curIndex===item }]"
        is-link
        v-for="item in num"
        :key="item"
      >
        <icon-bg slot="icon" :icon="deviceIcon" small></icon-bg>
        <span slot="title">{{ deviceName + item }}</span>
      </mt-cell>
    </mt-popup>
  </div>
</template>
...
export default {
  components: { MyPopup, MyLoading, MyPopup2, IconBg },
  data() {
    return {
      chartData: {},
      items: [],
